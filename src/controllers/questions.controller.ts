...
export const getFilteredQuestions = async (req: RequestUser, res: Response) => {
	try {
		const { category, ids, type, topic } = req.body as IGetFilteredQuestionsBody
		if (ids) {
			return res.status(200).json({ question: await Question.find({ _id: { $in: ids } }) })
			...
			return res
				...
				.json({
					questions:
						category.length === 0 ? await Question.find() : await Question.find({ topic: { $in: topic } }),
				})
		} else if (category) {
			return res
				.status(200)
				.json({
					questions:
						category.length === 0
							? await Question.find()
							: await Question.find({ category: { $in: category } }),
				})
		} else if (type) {
...
export const updateQuestion = async (req: RequestUser, res: Response) => {
	try {
		...
		const updatedQuestion = await Question.findByIdAndUpdate(
			req.params.id,
			{ content, scenario, category, parent, topic },
			{ new: true }
		)
...
export const postQuestion = async (req: RequestUser, res: Response) => {
	try {
		const { type, content, scenario, category, parent, topic } = req.body as IQuestion<
			SBAQuestion | ECQQuestion | CBQQuestion
		>

		if (!type || !content || !category || !scenario)
			return res.status(400).json({ mesage: "Uncompleted information" })
...
export const postReviewQuestion = async (req: RequestUser, res: Response) => {
	try {
		...
		if (!questionId || !review || !rate) {
			return res
				.status(400)
				.json({ message: "Must provide all the parameters", status: ResponseStatus.BAD_REQUEST })
		}
		...
		if (!(await Question.findById(questionId))) {
			return res
				.status(404)
				.json({
					message: "Question not found with the provided id",
					status: ResponseStatus.NOT_FOUND_QUESTIONS,
				})
		}
		...
		return res
			...
			.json({
				...
				status: ResponseStatus.RESOURCE_CREATED,
				questionReview,
			})
	} catch (error) {
		return res.status(500).json({ message: "Internal server error" })
...
export const getReviewQuestions = async (req: RequestUser, res: Response) => {
	try {
		...
		if (questionReviews.length === 0) {
			return res
				.status(404)
				.json({ message: "Question reviews not found", status: ResponseStatus.NOT_FOUND_REVIEW })
		}
	...
	try {
		...
		if (userRole.name === "User" && questionReview.authorId !== req.user._id) {
			return res
				.status(403)
				.json({
					message: "Not authorized to delete an alien question review",
		...
		if (!deletedQuestionReview) {
			return res
				.status(404)
				.json({
					message: "Question review not found with the provided id",
					status: ResponseStatus.NOT_FOUND_REVIEW,
				})
		}
		...
		return res
			.status(204)
			.json({
				message: "Question review deleted successfully",
				questionReview: deletedQuestionReview,
				status: ResponseStatus.RESOURCE_DELETED,
			})
	} catch (error) {
		return res.status(500).json({ message: "Internal server error" })
	}
